...
import { encodeA1Z26, validateEncoderInput } from '../../utils/cipher';
...
export function Encoder({ batchMode = false, onClear }: EncoderProps) {
  ...
  useEffect(() => {
    ...
    // Validate the input
    const { isValid, suggestion } = validateEncoderInput(input);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Text to Number Encoder</CardTitle>
      </CardHeader>
      <CardContent>
            ...
            <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md min-h-[50px] break-words">
              {output ? (
                ...
                <p className="text-gray-400 dark:text-gray-500">
                  Encoded numbers will appear here
                </p>
              )}
            </div>
          </div>
